...
describe("draw card", () => {
  ...
  it("draws a random card from the deck, excludes cards in `excludeCard` param", () => {
...
describe("draw multiple cards", () => {
  ...
  it("when provided array with card to exclude, does not draw those cards, has no repeated cards", () => {
    const deck = drawCardsFromDeck(52, [CODEX_DECK[0]]);
    const uniqueOnly = new Set(deck);
    expect(deck.length).toEqual(51);
    expect(uniqueOnly.size).toEqual(51);
  });
  it("when provided array with cards to exclude, does not draw those cards, has no repeated cards", () => {
    for (let index = 1; index < 52; index++) {
      const excludeCards = drawCardsFromDeck(index);
      const deck = drawCardsFromDeck(52, excludeCards)
        .map((card) => ({
          suit: card.suit,
          value: card.value
        }))
      ...
      expect(deck).toEqual(Array.from(uniqueOnly).sort());
    }
  });
  it("when draw the whole deck, should match original global constant", () => {
    ...
    const indexOnlyCodex = CODEX_DECK.map(
      (codexCard) => codexCard.index
    ).sort();
